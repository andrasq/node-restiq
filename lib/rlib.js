...
module.exports = (function() {
    var paramDecoders = {
        ...
        // TODO: A: ...but that is 10% slower than read()
...
// FIXME: tentative
                if (!type) {
                    ...
                    else if (isBase64(req.body, 10000)) type = 'base64';
    ...
    function readBody( req, res, next ) {
        ...
        function gatherChunk(chunk) {
            ...
            // use strings as long as node still supports a 'binary' conversion
            if (!readBinary) data += chunk.toString();
            //else data += chunk.toString('binary');
// FIXME: what if the chunk splits a multi-byte utf-8 character?
// FIXME: gaah!  "binary" converts to utf-8 from iso-latin-1 !! (and is not "binary")
// FIXME: eg new Buffer(new Buffer([0xc7]).toString("binary") + new Buffer([0xa2]).toString("binary"))' === <Buffer c3 87 c2 a2>
// TODO: Buffer.concat(array-of-chunks) is 7% slower than string append
        ...
        req.on('end', function() {
            req._bodyEof = true;
            if (0) req.body = readBinary ? new Buffer(data, 'binary') : data;
            // testing: set the body from whichever implementation gathered the chunks
            else req.body =
                (!readBinary) ? data :
                chunks.length ? Buffer.concat(chunks) :
