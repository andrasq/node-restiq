...
QRoute.prototype.mapRoute = function mapRoute( routeName ) {
    ...
    if ((route = this._literals[routeName])) {
        return {path: routeName, name: route.name, tail: tail, vars: {}, handlers: route.handlers};
    }
    ...
    for (i=0; i<patterns.length; i++) {
        ...
        if ((match = routeName.match(patterns[i].patt))) {
            ...
            return {path: routeName, name: patterns[i].name, tail: match[j+1], vars: vars, handlers: patterns[i].handlers};
        }
    }
    return undefined;
};
