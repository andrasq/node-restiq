...
Restiq.prototype.setServer = function setServer( server ) {
    this._server = server;
    return this;
};

Restiq.buildRouteName = function buildRouteName( method, path ) {
    if (!method || !path) throw new Error("buildRouteName: method and path are required");
...
Restiq.createServer = function createServer( opts ) {
    ...
    function try_run( handler, req, res, next ) {
        ...
        try { handler(req, res, next); } catch (err) { next(err); }
    }
    ...
    var server = http.createServer( function(req, res) {
        ...
        rlib.readBody(req, res, function(err) {
            ...
            //rlib.parseQueryParams(req, res);
            //rlib.parseBodyParams(req, res);
            ...
            var middlewareStack = restiq._pre.concat(route.handlers, restiq._post);
            aflow.applyVisitor(
                middlewareStack,
                function(handler, cb) {
...
Restiq.prototype.addRoute = function addRoute( method, path, handlers ) {
    // TODO: separate GET vs POST. vs Other routes, for much faster lookup
    ...
    if (method === 'GET') {
        ...
        var routeName = Restiq.buildRouteName(method, path);
...
Restiq.prototype.mapRoute = function mapRoute( method, url ) {
    ...
    if (method === 'GET') {
        ...
        var routeName = Restiq.buildRouteName(method, url);
...
app.pre(rlib.parseQueryParams);
app.pre(rlib.parseBodyParams);
