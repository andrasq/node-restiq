...
function finishMiddlewareStack( app, req, res, err ) {
    ...
    if (err) {
        // TODO: mimic the restify route a bit, this is just a placeholder
        if (app._emulateRestify) try { app.emit('uncaughtException', req, res, restifyRoute, err) } catch (e) { }
        return app._endWithError(req, res, err);
    }
...
Restiq.prototype._endWithError = function _endWithError( req, res, err, next ) {
    ...
    if (this._errorHandler) {
        this._errorHandler(req, res, err, function(err2) {
            if (err2) console.log("restiq: error in user errorHandler:", err2);
            if (next) next(err);
            return err;
        });
    }
