...
http.ServerResponse.prototype._body = null;

// accelerate access
http.IncomingMessage.prototype = toStruct(http.IncomingMessage.prototype);
http.ServerResponse.prototype = toStruct(http.ServerResponse.prototype);
...
function Restiq( opts ) {
    if (this === global || !this) return Restiq.createServer(opts);
    ...
    if (this._emulateRestify) {
        ...
        restify.addRestifyErrorHandler(this);
        this._setup.push(restify.emulateRestify);
    }

    // disable listen on new Restiq() to allow using Restiq() as an app builder
    this.listen = function() { throw new Error("new Restiq() cannot listen, use Restiq.createServer()"); }
}
...
Restiq.prototype.setTimeout = function setTimeout( timeout, listener ) {
    this._server.setTimeout(timeout, listener);
};
    ...
    app._server = createServer( function(req, res, whenDone) {
        ...
        setImmediate(function() {
            runMiddlewareStack(self, self._setup, req, res, function(err) {
                ...
                if (err) return finishMiddlewareStack(self, req, res, err, whenDone);

                // route the call
                var route = self.mapRoute(req.method, req.url);
// TODO: unmapped route should be 404 Not Found, but might be a breaking change
                if (!route) return finishMiddlewareStack(self, req, res, new self.errors.ErrorMethodNotAllowed("route not mapped"), whenDone);
                ...
                if (route._route.stack) {
                    ...
                    self._before.limit = route._route.steps;
                ...
                runMiddlewareStack(self, middlewareStack, req, res, function(err) {
                    // _finally steps are run by finishMiddlewareStack
                    // if mocking the server, will be provided with a callback
                    finishMiddlewareStack(self, req, res, err, whenDone);
                });
            });
        });
    });
...
// start the service, invoke the callback once started
...
Restiq.prototype.listen = function listen( ) {
    ...
    args.push(function(err) {
        if (confirmationCallback) confirmationCallback(err);
    });
...
Restiq.prototype.close = function close( cb ) {
    // suppress the error thrown on double-close
    try { this._server.close(cb) }
    catch (err) { if (cb) cb(err); else if (err.message.indexOf('Not running') >= 0) { } else { throw err; } }
};

function concatArrays( /* VARARGS */ ) {
    var i, j, dst = new Array(), nargs = arguments.length;
    for (i=0; i<nargs; i++) {
        var arr = arguments[i], len = (arr.limit >= 0 ? arr.limit : arr.length);
...
function finishMiddlewareStack( app, req, res, err, whenDone ) {
    // TODO: what route to emit?
    // TODO: can end up in here before routing, when no route will be set!
    var restifyRoute = req._route ? req._route.name : req.url;

    if (err) app._endWithError(req, res, err);

    // the _finally steps are executed after every call, whether error or not
    runMiddlewareStack(app, app._finally, req, res, function(err2) {

        // fully consume request body, for connection reuse
        if (!req._bodyEof) app.mw.discardBody(req, res);

        if (err || err2) {
            // TODO: mimic the restify route a bit better, this is just a placeholder
            if (app._emulateRestify) try { app.emit('uncaughtException', req, res, restifyRoute, err) } catch (e) { }
            return app._endWithError(req, res, err || err2);
            if (err && err2) console.log("restiq: mw error in 'finally' stack:", err2.stack)
        }
        // restify suppresses errors thrown in `after`
        if (app._emulateRestify) try { app.emit('after', req, res, restifyRoute, err); } catch(e) { }
        ...
        // Maybe could set a timeout to force an end after a minute.
...
Restiq.prototype._endWithError = function _endWithError( req, res, err ) {
    var self = this;
    ...
    // if the response has already been sent, cannot instead send an error
    if (res.headersSent) return;

    // generate an http error response
    if (this._errorHandler) {
        // use the provided errorHandler to generate an http error response
        this._errorHandler(req, res, err, function(err2) {
            if (err2) console.log("restiq: mw error in service errorHandler:", err2.stack);
        });
    }
    else {
        // use built-in logic to extract an http error response from err
        var code = 500, message = "middleware error";
        if (err.statusCode >= 100 && err.statusCode <= 999) { code = err.statusCode, message = err.message; }
        else if (err.code >= 100 && err.code <= 999) { code = err.code, message = err.message; }
...
Restiq.prototype.addStep = function addStep( func, where ) {
    if (typeof func === 'string') { var t = where; where = func; func = t; }
    where = where || 'use';
