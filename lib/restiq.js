...
http.IncomingMessage.prototype._bodyEof = false;
...
http.IncomingMessage.prototype.header = null;
http.ServerResponse.prototype.header = null;
...
Restiq.prototype.addStep = function addStep( func, where ) {
    ...
    for (var i=0; i<func.length; i++) {
        switch (where) {
        case 'before':
        case 'setup':   this._setup.push(func[i]); break;
        // 'pre' and 'use' share the _before mw stack, for now
        // FIXME: 'pre' should be the same as 'setup'
        case 'pre':     this._before.splice(this._preCount++, 0, func[i]); break;
        case 'use':     this._before.push(func[i]); break;
        case 'after':   this._after.push(func[i]); break;
        case 'finally': this._finally.push(func[i]); break;
...
// TODO: deprecate this method, this is not part of restify
...
function addRestifyMethodsToReqRes( req, res ) {
    ...
    res.send = function res_send( statusCode, body ) {
        ...
        if (typeof body !== 'object') encoding = 'text/plain';
            ...
            // TODO: convert generic error into restify error object
        }
        else if (typeof body.length === 'number' && Buffer.isBuffer(body)) encoding = 'application/octet-stream';
        else encoding = 'application/json';

        if (!this.headersSent) {
            this.writeHead(statusCode, {'Content-Type': encoding});
        }
        // TODO: use registered encoders for encoding
        if (encoding === 'application/json') this.end(JSON.stringify(body));
        else if (encoding === 'text/plain') this.end(body);
        else if (encoding === 'application/octet-stream') this.end(body.toString('base64'));
        else this.end(body);
    };
    ...
    res.json = function res_json(statusCode, body) {
        if (body === undefined && typeof statusCode !== 'number') { body = statusCode; statusCode = this.statusCode ? this.statusCode : 200; }
    ...
    req.version = function req_version() {
        return this.restiq.opts._version;
    };
    req.header = function req_header(name, defaultValue) {
        var value = this.headers[name.toLowerCase()];
        return (value || value !== undefined) ? value : defaultValue;
    };
    req.accepts = function req_accepts(type) {
        var value = this.headers['accepts'];
        // TODO: match 'html' against 'text/html', etc
        function trim(s) { return s.trim(); }
        return value ? value.split(';').map(trim).indexOf(type) : false;
    };
    // TODO:
    // is(type)
    // isSecure()
