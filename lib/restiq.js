...
Restiq.prototype.mapRoute = function mapRoute( method, url ) {
    var mappedRoute;
    if (method === 'GET') {
        mappedRoute = this._routesGet.mapRoute(url);
    }
    else if (method === 'POST') {
        mappedRoute = this._routesPost.mapRoute(url);
    }
    else {
        var routeName = method.toUpperCase() + '::' + url;
        mappedRoute = this._routesOther.mapRoute(routeName)
        // strip off the prepended
        mappedRoute.path = mappedRoute.path.slice(mappedRoute.path.indexOf('::') + 2);
        mappedRoute.name = mappedRoute.name.slice(mappedRoute.name.indexOf('::') + 2);
    }
    return mappedRoute;
};
