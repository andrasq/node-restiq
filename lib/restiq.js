...
http.IncomingMessage.prototype.headers = new Array();
...
Restiq.prototype.acceptable = [
    ...
    'application/javascript',                   // json object
...
// FIXME: ab with just 1 connections is 80% slower than restify (550 vs 1000), but with 8 is 50% faster ?! (2000 vs 1300 -t8)
// (and ab -t8 -c64 is 2800/s, 40% faster still (faster as in more throughput))

// FIXME: this new send() is much faster for kds (20%, 2800/s vs 2300/s),
// but slower for benchmark.js emulate (-7%, 17.7k/s vs 19k/s)
//console.log("AR: accept", this.getHeader('accept'));
        ...
        if (!this.headersSent) {
            // avoid setHeader, it slows response rates 15%
