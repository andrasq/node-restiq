...
Restiq.createServer = function createServer( opts ) {
    ...
    function adaptResToRestify( app, req, res ) {
        ...
        res.send = function(statusCode, object) {
            ...
            else if (!object) object = statusCode;
            if (!type) {
                ...
                else if (typeof object === 'string') type = 'text/plain';               // text
                else type = 'application/json';                                         // object
                if (typeof statusCode === 'number')
                    this.writeHead(statusCode, {'Content-Type': type});
                else {
                    if (!object) object = statusCode;
                    this.setHeader('Content-Type', type);
                }
            }
            if (!object && typeof statusCode !== 'number') object = statusCode;
